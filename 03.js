...
const MathX = (function () {
  ...
  const calculateFibbonachiRow = (num) => {
    ...
    for (let i = 2; i <= num; i++) {
      ...
      // Поэтому чтобы найти всех числа до числа N необходимо находить 2 предыдущих его числа
      f[i] = f[i - 1] + f[i - 2];
    }
  ...
  const calculatePrimeNumbers = (num) => {
    ...
    let primes = [];
    // Создадим массив, в котором будем "помечать" числа как false, а потом менять их значение
    ...
    for (let p = 2; p < maxSize; p++) {
      ...
      if (numbers[p] == true) {
        ...
        primes.push(p);
        // В случае, если переданное число равно количеству найденных - возвращаем его
